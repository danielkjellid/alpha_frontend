    ...
    <nuxt-link :to="`${product.slug}/`">
      <div class="product-media relative overflow-hidden bg-gray-100">
        ...
        <div class="absolute bottom-0 left-0 mb-6">
          <div v-show="product.site_state.can_be_purchased_online" class="py-1 pl-2 pr-3 text-sm font-medium text-white bg-gray-900 bg-opacity-75 rounded-r-full">Kan kjøpes på nett</div>
      ...
      <div class="product-info pt-4 pb-1">
        <div v-if="!showVariants" class="flex items-center justify-between">
          ...
          <p v-if="product.site_state.display_price" class="w-1/2 text-sm font-medium text-right text-gray-900">fra {{ product.site_state.gross_price | formatPrice }} <span class="text-gray-500">{{ product.unit }}</span></p>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const indexBasedOnBreakpont = (): number => {
    ...
    const amountOfVariants = computed(() => {
      const amount: number = (props.product.variant.length)-(indexBasedOnBreakpont())
      return amount
    })
